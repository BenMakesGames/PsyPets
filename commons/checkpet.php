...
//      mail('ben@telkoth.net', 'Virtual Hide-and-go-Seek Tag', $newpet['petname'] . ' will give it a try!', 'From: sender@telkoth.net' . "\n");
  ...
  if($newpet['zombie'] == 'yes' && mt_rand(1, 15) <= $break_tool)
    deteriorate_zombie($newpet, $myuser);
...
function eat_something(&$mypet, $hour)
{
  ...
  if(count($MATERIALS_LIST) > 0)
  {
    ...
    foreach($item_list as $itemname=>$quantity)
    {
      ...
      $made_of = take_apart(',', $this_item['recycle_for']);
      ...
      if(
        ...
        (
          ...
          $favorite_flavor == $item['itemname'] ||                           // if the item is the pet's favorite flavor
      ...
      )
      {
        ...
        $caffeine_gain += gain_caffeine($mypet, $item_item['ediblecaffeine']);
...
function deteriorate_zombie(&$zombie, &$owner)
{
  global $PET_SKILLS;
