...
$command = 'SELECT COUNT(*) AS c FROM monster_inventory WHERE user=' . quote_smart($user['user']) . ' AND location=\'storage\'';
...
$inventory = get_room_inventory($user['user'], 'storage', $num_items, $num_pages, $page, $user['storagesort']);
...
$footer_note .= '<br />Took ' . round($query_time, 4) . 's fetching storage inventory, and ' . round($count_time, 4) . 's counting the total bulk.';
...
?>
 <head>
  <title><?= $SETTINGS['site_name'] ?> &gt; <?= $user['display'] ?>'s Storage</title>
<?php include 'commons/head.php'; ?>
<?php include 'commons/ajaxinventoryjs.php'; ?>
 </head>
 <body>
<?php include 'commons/header_2.php'; ?>
     <h4 class="nomargin"><?= $user['display'] ?>'s Storage <i>(<span class="housebulk"><?= $size_report ?></span> <a href="storagesummary.php"><img src="gfx/summary.png" width="18" height="16" alt="(summary)" border="0" /></a>)</i></h4>
<?php
echo '<p style="padding-left: 2em;"><a href="/autosort.php?applyto=storage">auto-sort items</a> | <a href="/autosort_edit.php">configure auto-sorter</a>';
...
?>
     <ul class="tabbed">
      <li class="activetab"><a href="/storage.php">Storage</a></li>
...
?>
    ...
    <input type="hidden" name="from" value="storage" />
<?php
if(count($inventory) > 0)
{
  if($page_note)
  {
    echo '<p><i>(You have over 2000 items in storage!  When this happens, ' . $SETTINGS['site_name'] . ' paginates your Storage display.)</i></p>';
    echo paginate($num_pages, $page, '/storage.php?page=%s');
  }
?>
<table>
 <tr>
  <td>
   <input type="button" value="Move to" onclick="move_items('move')" id="moveto1" />&nbsp;<select id="move1" name="move1" onchange="document.getElementById('move2').selectedIndex = this.selectedIndex">
<?php
...
?>
  ...
  <td><input type="button" value="Gamesell" onclick="sell_items('<?= $user['license'] ?>')" id="gamesell1" /></td>
  <td><input type="button" value="Throw Out" onclick="trash_items()" id="throwout1" /></td>
 </tr>
</table>
<?php
...
<table>
 <tr>
  <td>
   <input type="button" value="Move to" onclick="move_items('move')" id="moveto2" />&nbsp;<select id="move2" name="move2" onchange="document.getElementById('move1').selectedIndex = this.selectedIndex">
<?php
  foreach($rooms as $room)
    echo '    <option value="' . $room . '">' . ($room{0} == '$' ? substr($room, 1) : $room) . '</option>' . "\n";
?>
  ...
  <td><input type="button" value="Gamesell" onclick="sell_items('<?= $user['license'] ?>')" id="gamesell2" /></td>
  <td><input type="button" value="Throw Out" onclick="trash_items()" id="throwout2" /></td>
 </tr>
</table>
<?php
  if($page_note)
  ...
  echo '<p>Your storage is empty.</p>';
?>
     </form>
