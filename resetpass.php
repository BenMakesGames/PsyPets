...
if($_POST["submit"] == "Reset Password")
{
  $this_user = $database->FetchSingle('
    SELECT *
    FROM monster_users
    WHERE `user`=' . quote_smart(trim($_POST["user"])) . '
    AND `email`=' . quote_smart(trim($_POST["email"])) . '
    LIMIT 1
  ');

  if($this_user === false)
    $error_message = 'Login name and e-mail address do not match.';
  else
  {
...
?>
     <p>If you forgot your password, you can have instructions on how to reset your password e-mailed to you.</p>
     <p>Enter the login name and e-mail address of the account to get started:</p>
     <form method="post">
     <table>
      <tr>
       <td>Login name:</td>
       <td><input name="user" maxlength="16" value="<?= htmlentities($_POST['user_lookup']) ?>" /></td>
      </tr>
      <tr>
       <td>E-mail:</td>
       <td><input name="email" value="<?= htmlentities($_POST['email_lookup']) ?>" /></td>
      </tr>
     </table>
