...
if($this_inventory['itemname'] == 'Sated Shovel')
  $max_level = 6;
else //ex: "Hungry Shovel (level 4)"
...
if($_POST['action'] == 'feed' && $max_level < 6 && $last_feed_time + $time_between_feeding <= $now)
{
    ...
    && substr($details['itemtype'], 0, 5) != 'food/' && substr($details['itemtype'], 0, 6) != 'plant/')
  {
    ...
    if($food_value > 0)
    {
      echo 'The Hungry Shovel devours the ' . $item['itemname'] . ' in one bite, and grins.';

      $exp += $food_value;
      $new_level = $max_level;
      while($exp >= $exp_needed && $new_level < 6)
      {
        $new_level++;
        $exp -= $exp_needed;
      ...
      if($new_level > $max_level)
      {
        ...
        if($new_level == 6)
        {
          echo '</p><p><i>(The Hungry Shovel has been sated!)</i>';

          $extra = ',itemname=' . quote_smart('Sated Shovel');
        }
        else
        {
          echo '</p><p><i>(The Hungry Shovel has increased in level!)</i>';

          $extra = ',itemname=' . quote_smart('Hungry Shovel (level ' . $new_level . ')');
        }
      }
      else
        echo '</p><p>';
    ...
    {
      echo 'The Hungry Shovel devours the ' . $item['itemname'] . ' in one bite, coughs a little, and spits up some Rubble.  It apparently didn\'t like the meal you gave it.';
      add_inventory($user['user'], '', 'Rubble', 'Coughed up by a Hungry Shovel', $this_inventory['location']);
    }
    ...
    foreach($inventory as $item)
    {
      if($item['idnum'] != $this_inventory['idnum'])
      {
          ...
          && substr($details['itemtype'], 0, 5) != 'food/' && substr($details['itemtype'], 0, 6) != 'plant/')
        {
          ...
          if($items == 1)
          {
            echo '<i>(A Hungry Shovel can eat a variety of items, however not all are good for them.  Note that the quantity listed is the number available in your house, not the number you will feed.  You will always only give the Hungry Shovel 1 item to eat.)</i></p>' .
                 '<form method="post">' .
                 '<table>' .
                 '<tr class="titlerow">' .
...
<?php
    ...
    if($items > 0)
    {
      ...
      echo 'There are no items here to feed to the Hungry Shovel.';
  }
  else
    echo 'This Hungry Shovel is sated... for the moment.';
}
?>
