...
<?php
  ...
  else
  {
    ...
    if($target_pet['graphic_size'] < 48)
    {
      $new_size = min(48, $target_pet['graphic_size'] + mt_rand(4, 8));
    
      $database->FetchNone('UPDATE monster_pets SET graphic_size=' . (int)$new_size . ' WHERE idnum=' . $target_pet['idnum'] . ' LIMIT 1');

      echo '<p>' . $target_pet['petname'] . ' increases in size in a series of abrupt spurts!</p>';
    }
    else
    {
      echo '<p>The ' . $target_pet['petname'] . ' increases in size in a series of abrupt spurts, but then, after a moment of nothing happening, slowly shrinks - as if being deflated - to ' . his_her($target_pet['gender']) . ' original size!</p>';
    }
