...
$consider_item = get_smith_byid($action_info[2]);
...
if(count($unavailable) == 0)
{
  if($_GET['step'] == 2)
  {
    ...
    $command = 'INSERT INTO monster_projects (`type`, `userid`, `creator`, `projectid`, `progress`, `notes`) ' .
               "VALUES ('smith', " . $user['idnum'] . ', \'u:' . $user['idnum'] . '\', ' . $consider_item['idnum'] . ", '0', 'You started this smithing project.')";
    $database->FetchNone($command, 'adding project from blueprint');

    echo 'You set up the foundations for the ' . $consider_item['makes'] . ' project.';
...
{
?>
This blueprint details the construction of <?= $consider_item['makes'] ?>.</p>
<p>You will need:</p>
<ul>
<?php
