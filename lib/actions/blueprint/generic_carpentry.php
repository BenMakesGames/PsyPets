...
$consider_item = get_carpentry_byid($action_info[2]);
...
if(count($unavailable) == 0)
{
  if($_GET['step'] == 2)
  {
    ...
    $command = 'INSERT INTO monster_projects (`type`, `userid`, `creator`, `projectid`, `progress`, `notes`) ' .
               "VALUES ('carpenter', " . $user['idnum'] . ', \'u:' . $user['idnum'] . '\', ' . $consider_item['idnum'] . ", '0', 'You started this carpentry project.')";
...
{
?>
<p>This blueprint contains instructions for making <?= $consider_item['makes'] ?>.</p>
<p>You will need:</p>
<ul>
<?php
