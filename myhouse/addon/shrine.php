...
<?php
    ...
    {
      if(strlen($candles[$x]) == 0)
        echo '<option value="' . $x . '">' . Romanize($x + 1) . '</option>';
      else
        echo '<option disabled>' . Romanize($x + 1) . '</option>';
    }
?>
     </select>
     <select name="candle">
<?php
    foreach($inventory as $itemname=>$data)
      echo '<option value="' . $candle_data[$itemname]['idnum'] . '">' . $itemname . ' (' . $data['c'] . ')</option>';
?>
     </select>
...
<?php
    ...
    echo '<p>There are no Candles available in your house. (Candles in protected rooms are not available here.)</p>';
}
else
  echo '<p>There are no free spaces in your Shrine.  You\'ll have to wait until a candle burns out.</p>';
?>
     <h5>Spells</h5>
<?php
if(count($spells) > 0)
...
<?php
  ...
  foreach($spells as $spell)
  {
    ...
    if(array_key_exists($data[0], $SPELL_DETAILS))
    {
      $casts = 0;
      $built_up = $data[1];
...
<?php
  ...
  echo '<p>No spells are in progress.</p>';
