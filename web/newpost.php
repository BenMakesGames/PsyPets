...
<body onload="consider_action();">
        ...
        <div class="plazapost" style="width:100%; border-color:#<?= $user['color'] ?>;">
            <div style="background: url(/gfx/residentbar.png) repeat-y;">
                <div class="plazaposttitlebar" style="color:<?= $text_color ?>;background-color:#<?= $user['color'] ?>;">
                    ...
                    <div class="plazaposttitle">
                        <a href="/residentprofile.php?resident=<?= link_safe($user['display']) ?>" style="color:<?= $text_color ?>;"><?= $user['display'] ?></a>
                        <select name="says">
                            <?php foreach($THREAD_SAYS_ALLOWED as $says): ?>
                                <option value="<?= $says ?>"><?= $says ?></option>
                            <?php endforeach; ?>
                        </select>:
                        <input name="title" style="width:450px;" maxlength="120" value="<?= htmlspecialchars($title) ?>" placeholder="post title, optional" />
                    </div>
                </div>
                <div>
                    <div class="plazapostresident centered">
                        <a href="/residentprofile.php?resident=<?= link_safe($user['display']) ?>"><img src="<?= user_avatar($user) ?>" alt="" width="48" height="48" /></a><br />
                        <?php if($user['badges'] != ''): ?>
                            <?php $badges = explode(',', $user['badges']); ?>
                            <?php foreach($badges as $badge): ?>
                                <img src="//<?= $SETTINGS['static_domain'] ?>/gfx/badges/<?= $badge ?>.png" title="<?= $BADGE_DESC[$badge] ?>" alt="<?= $BADGE_DESC[$badge] ?>" width="20" height="20" />
                            <?php endforeach; ?>
                        <?php endif; ?>

                        <div>
                            <a href="/writemail.php?sendto=<?= link_safe($user['display']) ?>"><img src="/gfx/sendmail.gif" width="16" height="16" title="Send Mail" alt="send mail" /></a>

                            <?php if($user['license'] == 'yes'): ?>
                                <a href="/newtrade.php?user=<?= link_safe($user['display']) ?>"><img src="/gfx/dotrade.gif" width="16" height="16" title="Initiate Trade" alt="start trade" /></a>
                            <?php endif; ?>
                            <?php if($user['openstore'] == 'yes'): ?>
                                <a href="/userstore.php?user=<?= link_safe($user['display']) ?>"><img src="/gfx/forsale.png" width="16" height="16" alt="Visit Store" title="Visit Store" /></a>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <?php foreach($previous_posts as $p_post): ?>
        <?php render_post($p_post, $replying_thread, $plazainfo); ?>
    <?php endforeach; ?>
