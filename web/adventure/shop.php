...
if($_POST['exchange'] == 1 && $challenge_tokens['copper'] >= 3)
{
  $challenge_tokens['copper'] -= 3;
  $challenge_tokens['silver']++;
  
  update_challenge_tokens($challenge_tokens);
  
  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 2 && $challenge_tokens['silver'] >= 3)
{
  $challenge_tokens['silver'] -= 3;
  $challenge_tokens['gold']++;

  update_challenge_tokens($challenge_tokens);

  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 3 && $challenge_tokens['gold'] >= 3)
{
  $challenge_tokens['gold'] -= 3;
  $challenge_tokens['platinum']++;

  update_challenge_tokens($challenge_tokens);

  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 4 && $challenge_tokens['plastic'] >= 3)
{
  $challenge_tokens['plastic'] -= 3;
  $challenge_tokens['copper']++;
  
  update_challenge_tokens($challenge_tokens);

  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 5 && $challenge_tokens['silver'] > 0)
{
  $challenge_tokens['silver']--;
  $challenge_tokens['copper'] += 2;
  
  update_challenge_tokens($challenge_tokens);
  
  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 6 && $challenge_tokens['gold'] > 0)
{
  $challenge_tokens['gold']--;
  $challenge_tokens['silver'] += 2;

  update_challenge_tokens($challenge_tokens);

  $dialog_exchanged = true;
}
else if($_POST['exchange'] == 7 && $challenge_tokens['platinum'] > 0)
{
  $challenge_tokens['platinum']--;
  $challenge_tokens['gold'] += 2;
...
if($_POST['submit'] == 'Buy')
{
  ...
  {
    if($challenge_tokens[$store[$buy][2]] >= $store[$buy][1])
    {
      $challenge_tokens[$store[$buy][2]] -= $store[$buy][1];
      update_challenge_tokens($challenge_tokens);
...
<?php include 'commons/header_2.php'; ?>
     ...
     <ul class="tabbed">
      <li><a href="/adventure/">My Adventure</a></li>
      <li class="activetab"><a href="/adventure/shop.php">Adventurer's Shop</a></li>
     </ul>
<?php
...
foreach($tokens as $token)
{
  ...
  echo '<img src="/gfx/items/token_' . $token . '.png" alt="" /></td><td>' . ucfirst($token) . '</td><td>x ' . $challenge_tokens[$token] . '</td>' . "\n";
}
?>
      </tr>
     </table>
...
if($challenge_tokens['plastic'] >= 3)
  $exchanges[] = '<input type="radio" name="exchange" value="4" /> Trade 3 Plastic Tokens for 1 Copper Token';

if($challenge_tokens['copper'] >= 3)
  $exchanges[] = '<input type="radio" name="exchange" value="1" /> Trade 3 Copper Tokens for 1 Silver Token</a>';

if($challenge_tokens['silver'] > 0)
  $exchanges[] = '<input type="radio" name="exchange" value="5" /> Trade 1 Silver Token for 2 Copper Tokens</a>';
if($challenge_tokens['silver'] >= 3)
  $exchanges[] = '<input type="radio" name="exchange" value="2" /> Trade 3 Silver Tokens for 1 Gold Token</a>';

if($challenge_tokens['gold'] > 0)
  $exchanges[] = '<input type="radio" name="exchange" value="6" /> Trade 1 Gold Token for 2 Silver Tokens</a>';
if($challenge_tokens['gold'] >= 3)
  $exchanges[] = '<input type="radio" name="exchange" value="3" /> Trade 3 Gold Tokens for 1 Platinum Token</a>';

if($challenge_tokens['platinum'] > 0)
  $exchanges[] = '<input type="radio" name="exchange" value="7" /> Trade 1 Platinum Token for 2 Gold Tokens</a>';

if(count($exchanges) > 0)
  echo '<form method="post"><table><tr><td valign="middle">' . implode('<br />', $exchanges) . '</td><td valign="middle"><input type="submit" name="submit" value="Exchange" /></td></tr></table></form>';
?>
     <h5>Shop</h5>
<?php
if($message)
...
foreach($store as $id=>$details)
{
  ...
  if($challenge_tokens[$details[2]] >= $details[1])
  {
?>
      <tr class="<?= $rowclass ?>">
