...
?>
<head>
<title><?= $SETTINGS['site_name'] ?> &gt; Help &gt; Copyright Information &gt; Code Libraries</title>
...
<p><?= $SETTINGS['site_name'] ?> wouldn't be possible without open-source software, not just the stuff listed below, but Linux, Apache, GZIP, PNG, and hundreds of pieces of software I'm sure I'm not even aware of!</p>
...
<h5>How <?= $SETTINGS['site_name'] ?> Uses It</h5>
...
<p>jQuery is a pretty popular library among web developers, and is working its magic behind the scenes on a lot of the web pages you visit on a regular basis, <?= $SETTINGS['site_name'] ?> included!  (You visit regularly, right!? :P)</p>
</div>
...
<h5>How <?= $SETTINGS['site_name'] ?> Uses It</h5>
<p>wz_tooltips is another older JavaScript library, this one responsible for the tooltips on <?= $SETTINGS['site_name'] ?>.  Again, sure, a jQuery plugin could do this, but wz_tooltips was designed for the purpose, and avoids all the overhead that comes with working in jQuery.</p>
</div>
...
<p>The code has been modified by Ben Hendel-Doying for use in <?= $SETTINGS['site_name'] ?>.</p>
<h5>How <?= $SETTINGS['site_name'] ?> Uses It</h5>
...
<p>Used right, megamenus are very powerful; I hope you find <?= $SETTINGS['site_name'] ?> has made good use of the megamenu concept.</p>
</div>
...
<p>"Yeah... but so I just drag them bigger..."</p>
...
<p>This is mainly used for for <a href="/statistics.php">graphing certain site statistics</a>, but also for some internal administrative tools.  I also hope to apply it to <?= $SETTINGS['site_name'] ?> market data, but I keep getting distracted with other things :P</p>
</div>
...
<h5>How <?= $SETTINGS['site_name'] ?> Uses It</h5>
<p>jQuery Tablesorter lets tables be sorted without reloading the page!  (More sneaky JavaScript stuff!)  <?= $SETTINGS['site_name'] ?> doesn't use this nearly as much as it should, but it's getting there, one step at a time.</p>
<p>The most noteable place it's used at the moment is probably the <?= $SETTINGS['site_name'] ?> Mailbox.</p>
</div>
