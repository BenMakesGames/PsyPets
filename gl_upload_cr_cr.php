  ...
  else
  {
    ...
    else
    {
      $command = 'INSERT INTO `monster_graphicslibrary` (`uploader`, `w`, `h`, `recipient`, `title`, `author`, `rights`) VALUES ' .
                 '(' . $user['idnum'] . ", '$w', '$h', " . $recipient . ', ' . quote_smart($title) . ', ' . quote_smart($author) . ', \'unlimited\')';
      ...
      if(move_uploaded_file($_FILES['file']['tmp_name'], $base_path . '/' . $newfile))
      {
        ...
        psymail_user('telkoth', 'psypets', 'new graphics library content', '{r ' . $user['display'] . '} uploaded the following image (' . $newfile . '):<br /><img src="' . $newfile . '" /><br />They are claiming ownership, and releasing the graphic for limited use by PsyPets.');
...
{
?>
<p>Great!  Let me go over the details again real quick...</p>
<p>By uploading a graphic using this form, you are agreeing to the following:</p>
<ul>
...
?>
     ...
     <form enctype="multipart/form-data" action="gl_upload_cr_cr.php" method="post">
     <table>
      <tr>
       <th>Graphic title:</th>
